...
def cuantizacion_vectorial(path_imagen, diccionario = []):
    ...
    ratio_compresion = (8*n*m)/(bits_indice*num_bloques + bits_overhead)
    plt.figure()    
    plt.imshow(imagen_cuantizada, cmap=plt.cm.gray)
    titulo = 'Ratio: ' + str('%.5f' % ratio_compresion) + \
        '\nError: ' + str('%.5f' % sigma) + \
        '\nBits por píxel ' + str('%.5f' % bits_pixel)
    plt.title(titulo, fontsize=10)
    plt.show()
    return diccionario


diccionario = cuantizacion_vectorial('./standard_test_images/lena_gray_512.png')
...
"""
cuantizacion_vectorial('./standard_test_images/peppers_gray.png')
...
Calcular el error.
"""
cuantizacion_vectorial('./standard_test_images/peppers_gray.png', diccionario)
